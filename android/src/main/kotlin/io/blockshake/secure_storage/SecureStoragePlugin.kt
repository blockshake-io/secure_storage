...
class SecureStoragePlugin : FlutterPlugin, ActivityAware, MethodCallHandler {
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        ...
        try {
            ...
            fun BiometricStorageFile.withAuth(
                ...
                if (options.biometricAccessControl == BiometricAccessControl.biometryNone) {
                    return cb(null)
                }
            ...
            when (call.method) {
                ...
                "init" -> {
                    ...
                    val options = call.argument<Map<String, Any>>("options")?.let { it ->
                        var accessControl: BiometricAccessControl?;
                        if(it["biometricAccessControl"] == "biometryNone") {
                            accessControl = BiometricAccessControl.biometryNone;
                        } else if(it["biometricAccessControl"] == "biometryAny") {
                            accessControl = BiometricAccessControl.biometryAny;
                        } else if(it["biometricAccessControl"] == "biometryCurrentSet") {
                            accessControl = BiometricAccessControl.biometryCurrentSet;
                        } else {
                            result.success(false)
                            return
                        }
...
//                        ?: InitOptions()
    ...
    private fun authenticate(
        ...
        val biometricOnly =
            options.biometricAccessControl != BiometricAccessControl.biometryNone || Build.VERSION.SDK_INT < Build.VERSION_CODES.R
        ...
        if (cipher == null) {
            ...
            logger.debug { "Authenticating without cipher. ${options.biometricAccessControl}" }
